...
void do_snoop_operation(BusSnooper* snooper)
{
    ...
    if ((snooper->myCore->bus_manager->bus_cmd.now == BUS_RD ||
        snooper->myCore->bus_manager->bus_cmd.now == BUS_RDX) &&
        snooper->myCore->bus_manager->bus_origid.now != snooper->myCore->id)
    {
        uint32_t address = snooper->myCore->bus_manager->bus_addr.now;
        ...
        if (in_cache(address, &(snooper->myCore->cache_now)))
        {
            ...
            if (state == MODIFIED && snooper->myCore->bus_manager->bus_cmd.now == BUS_RD)
            {
                snooper->myCore->bus_manager->interuptor_id = snooper->myCore->id;
                snooper->myCore->bus_manager->Interupted.updated = true;
            ...
            if (state == MODIFIED && snooper->myCore->bus_manager->bus_cmd.now == BUS_RDX)
            {
                snooper->myCore->bus_manager->interuptor_id = snooper->myCore->id;
                snooper->myCore->bus_manager->Interupted.updated = true;
                update_state(address,
            ...
            if (snooper->myCore->bus_manager->bus_cmd.now == BUS_RDX)
            {
                update_state(address,
                             &(snooper->myCore->cache_updated),
                             INVALID);
            ...
            if (snooper->myCore->bus_manager->bus_cmd.now == BUS_RD)
            {
                update_state(address,
                             &(snooper->myCore->cache_updated),
                             SHARED);
                snooper->myCore->bus_manager->bus_shared.updated = true;
            }
        }
    }
}
